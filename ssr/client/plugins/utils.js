function offsetTop(obj) {
  ...
  if (obj.parentElement.tagName !== 'BODY') {
    top += Number(offsetTop(obj.parentElement));
  }
  return top;
}
  ...
  if (currentScroll < top) {
    (function smoothscroll() {
      ...
      if (currentScroll < top) {
        ...
        if (height === currentScroll) cancelAnimationFrame(timer);
        height = currentScroll;
        let dt = currentScroll / 10;
        dt = Math.min(dt, 100);
        dt = Math.max(dt, 10);
        const next = currentScroll + dt;
...
const formatDate = (str) => {
  const arr = ['一', '二', '三', '四', '五', '六', '七', '八', '九', '十', '冬', '腊'];
